...
export class WordSearchModule {
  ...
  render(root){
    ...
    if(seed && typeof seed === 'object'){
      ...
      const t = seed.title || '';
      const pid = seed.puzzleId || seed.sectionId || seed.sectionTitle || ''; // <-- prioridade puzzleId
      $('#ws_title').value = t;
      $('#ws_pid').value = pid ? normId(pid) : '';

      hint.textContent = `Recebido do Builder ✅ Agora clique "Gerar" e depois "Salvar Puzzle do Livro".`;
    } else {
      applyPreset();
    }
    ...
    const savePuzzle = () => {
      ...
      // ID estável (se veio do Builder, já vem preenchido)
      ...
      this.app.toast?.('Puzzle salvo no Livro ✅ (volte no Builder pra ver quadriculado)');
